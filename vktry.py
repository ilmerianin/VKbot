# слабо систематизированный источник кода отдельных процедур
# многие функции отсылают обратно 
...
while token_passed == 0:
    ...
    api = vk.API(access_token=token ,v='5.131', lang='ru')
    try:
        api.users.get()
        token_passed = 1
...
if not os.path.exists(token_dir):
    os.mkdir(token_dir)
os.chdir(token_dir) #смена рабочей директории имя токен я
...
def dl_attachments_from_dialog():
    clear()
    print(color.Cyan + "Данная функция будет позжеДля выхода в главное меню нажмите Q"  + color.END)
    choice = input('>>> ')
    if choice == 'Q' or choice == 'q':
        return
