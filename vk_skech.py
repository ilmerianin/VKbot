...
#  преобразоввть List в Dict по  ключевой позиции
def ist_to_dict(listA, keyl):
    ''' Преобразовывает лист состоящий из словарей в соварь с ключами поданными в keyl котрые берутся из словарей включенных в лист (словари друзей с id- которые и стали ключами  23.10.22'''
    dictB={x[keyl] : x  for x in listA}
    #pprint.pprint(dictB.keys()) # вывод ключей 
...
#Инфа по айди запрос по API
def info_by_id(api, user_id = 0,user_info =0):
    ''' выдаеь на экран инфу по акаунту человека 
        user_info :  dict
        если user_id не послан то спрашиваеь ввод user_id
        если не послал user_info то делает запрос через API
        работает запрашивает информацию по ID  20.10.22'''
    if user_id == 0:
        user_id = input(color.Yellow + 'ID пользователя >>> ' + color.Cyan)
    if user_info == 0:
        user_info = api.users.get(user_ids=user_id, fields='relation, activity, can_write_private_message, online, sex, counters')
    ...
    friends ='Друзей :'+ str( info.get('counters').get('friends'))
...
def page_info(api):
    ''' возможно нужно передать номер ID и подставить в запросы '''
